    ...
    return (
        <div className="min-h-screen bg-neutral-100 flex font-sans text-neutral-900">
            <aside
                className={`${
                    collapsed ? "w-20" : "w-64"
                } bg-black text-white flex-shrink-0 flex flex-col fixed h-full border-r border-neutral-800 transition-all duration-300 z-20 shadow-xl`}
            >
                <div
                    className={`h-20 flex items-center ${
                        collapsed ? "justify-center px-0" : "px-6"
                    } border-b border-neutral-800 relative`}
                >
                ...
                <nav className="flex-1 p-4 space-y-2 overflow-x-hidden">
                    ...
                    <div className={`pt-6 pb-2 text-xs font-bold text-neutral-600 uppercase tracking-widest transition-opacity duration-300 ${collapsed ? "opacity-0 h-0 p-0 overflow-hidden" : "px-4"}`}>
                        Catalogue
                    </div>
            ...
            <main className={`flex-1 ${collapsed ? "ml-20" : "ml-64"} p-8 overflow-y-auto transition-all duration-300`}>
                {children}
            </main>
        </div>
    );
